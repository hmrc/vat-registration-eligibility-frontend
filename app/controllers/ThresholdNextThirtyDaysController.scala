...
class ThresholdNextThirtyDaysController @Inject()(appConfig: FrontendAppConfig,
  ...
  def onPageLoad() = (identify andThen getData) {
    implicit request =>
      val preparedForm = request.userAnswers.flatMap(_.thresholdNextThirtyDays) match {
        case None => form
  ...
  def onSubmit() = (identify andThen getData).async {
    implicit request =>
      form.bindFromRequest().fold(
        ...
        (value) =>
          dataCacheConnector.save[Boolean](request.internalId, ThresholdNextThirtyDaysId.toString, value).map(cacheMap =>
            Redirect(navigator.nextPage(ThresholdNextThirtyDaysId, NormalMode)(new UserAnswers(cacheMap))))
      )
  }
}
