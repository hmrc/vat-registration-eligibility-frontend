  ...
  def onSubmit() = (identify andThen getData andThen requireData).async {
    implicit request =>
      form.bindFromRequest().fold(
        ...
        (value) =>
          dataCacheConnector.save[Boolean](request.internalId, ThresholdPreviousThirtyDaysId.toString, value).map(cacheMap =>
            Redirect(navigator.nextPage(ThresholdPreviousThirtyDaysId, NormalMode)(new UserAnswers(cacheMap))))
      )
  }
}
