...
class FakeDataRetrievalAction(cacheMapToReturn: Option[CacheMap]) extends DataRetrievalAction {
  override protected def transform[A](request: CacheIdentifierRequest[A]): Future[OptionalDataRequest[A]] = cacheMapToReturn match {
    case None => Future(OptionalDataRequest(request.request, request.cacheId, CurrentProfile("", "", None), None))
    case Some(cacheMap)=> Future(OptionalDataRequest(request.request, request.cacheId, CurrentProfile("", "", None), Some(new UserAnswers(cacheMap))))
  }
}