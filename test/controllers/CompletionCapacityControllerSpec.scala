...
import models.CompletionCapacity
...
class CompletionCapacityControllerSpec extends ControllerSpecBase {
  ...
  def viewAsString(form: Form[_] = form) = completionCapacity(frontendAppConfig, form, NormalMode)(fakeRequest, messages).toString
  ...
  "CompletionCapacity Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData = Map(CompletionCapacityId.toString -> JsString(CompletionCapacity.values.head.toString))
      val getRelevantData = new FakeDataRetrievalAction(Some(CacheMap(cacheMapId, validData)))

      val result = controller(getRelevantData).onPageLoad()(fakeRequest)

      contentAsString(result) mustBe viewAsString(form.fill(CompletionCapacity.values.head))
    }

    "redirect to the next page when valid data is submitted" in {
      val postRequest = fakeRequest.withFormUrlEncodedBody(("value", CompletionCapacity.options.head.value))
