...
class TurnoverEstimateControllerSpec extends ControllerSpecBase {
  ...
  "TurnoverEstimate Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData = Map(TurnoverEstimateId.toString -> JsString(TurnoverEstimate.values.head.toString))
      val getRelevantData = new FakeDataRetrievalAction(Some(CacheMap(cacheMapId, validData)))

      val result = controller(getRelevantData).onPageLoad(NormalMode)(fakeRequest)

      contentAsString(result) mustBe viewAsString(form.fill(TurnoverEstimate.values.head))
    }
